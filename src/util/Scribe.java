...
public class Scribe {
	...
	public String tabGuess(String input) 
	{
		...
		if (suggestionUsed) 
		{
			eval = input.toLowerCase();
			this.lastInput = input;
		}
		else 
		{
			eval = this.lastInput.toLowerCase();
			skip = true;
		}
		...
		if (evalLength > 0) {
			for(String a: Command.getAllTriggers()) {
				if(!suggestionUsed) {
					if(input.equals(a)){
						skip = false; 
						...
						if(skip) {
							...
							String diff = a.substring(0,evalLength);
							communicate(diff);
							communicate(a.substring(0,evalLength-1));
							if(diff.length()==0) diff += a.charAt(0);
							if(eval.equals(diff)) {
					...
					if(evalLength > a.length()) continue;
					
					String diff = a.substring(0, (evalLength<1)? 1:evalLength);
